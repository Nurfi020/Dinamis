...
export function generateRandomSegment(length: number = 4): string {
  // Exclude easily confused characters like O, 0, I, 1 for better human readability
...
function createInitialSeed(): DatabaseSchema {
  ...
  const licenses: License[] = [
    ...
    // 2. Production Sample License 1 (A7X9-K2MP-8Q4T)
    {
      id: 'lic-life-0001',
...
class LicenseDatabase {
  ...
  private loadFromDisk(): DatabaseSchema {
    try {
      ...
      console.warn('Could not read persistent license db from disk, using seed:', e);
    }
  ...
  private saveToDisk(data: DatabaseSchema) {
    try {
      ...
      console.warn('Could not persist license db to disk:', e);
    }
  }
  ...
  public saveToken(record: ActivationTokenRecord): void {
    // Remove existing tokens for the same license+device
